...
const UserDetail = () => {
  ...
  const fetchData = () => {
    ...
    axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/users/${id}`, config)
      .then(res => {
        setUser(res.data);
        setNewAddress(prev => ({ ...prev, userId: parseInt(id) }));
        setNewStudy(prev => ({ ...prev, userId: parseInt(id) }));
      })
      .catch(() => navigate('/users'));

    axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/Addresses/${id}`, config)
      .then(res => setAddresses(res.data.$values || []))
      .catch(console.error);

    axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/Studies/${id}`, config)
  ...
  const createAddress = (e) => {
    ...
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/Addresses`, addressData, config)
      .then(() => {
        ...
        setNewAddress({ calle: '', numero: '', ciudad: '', userId: parseInt(id) });
  ...
  const createStudy = (e) => {
    ...
    axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/Studies`, studyData, config)
      .then(() => {
        ...
        setNewStudy({ nombre: '', descripcion: '', userId: parseInt(id) });
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        <div className={styles.pageHeader}>
          <h1>Detalle de Usuario: {user.nombre} {user.apellido}</h1>
